...
export default function AdminStats() {
  ...
  const totalRevenue = (() => {
    ...
    confirmedRegistrations.forEach(reg => {
      ...
      if (!isLeader && reg.profile_id && reg.event?.id) {
        isTeamMember = confirmedRegistrations.some(
          otherReg =>
    ...
    // return revenue;
    return Math.max(0, revenue - 6000);
  })();
  ...
  const categoryStats = ["Cultural", "Technical", "Sports"].map(category => {
    ...
    const categoryRevenue = categoryRegs
      .filter(r => r.status === "confirmed")
      .reduce((sum, r) => sum + (r.event?.fee || 0), 0);
    return {
      category,
  ...
  const getEventStats = eventId => {
    ...
    const revenue = eventRegs
      .filter(r => r.status === "confirmed")
      .reduce((sum, r) => sum + (event?.fee || 0), 0);
    const pendingRev = eventRegs
    ...
    const topEventSchools = Object.entries(schoolDist)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    ...
    const topEventDepts = Object.entries(deptDist)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    ...
    if (event?.subcategory === "Group") {
      ...
      teamStats = {
        ...
        avgSize:
          teamSizes.length > 0
            ? (teamSizes.reduce((a, b) => a + b, 0) / teamSizes.length).toFixed(1)
            : 0,
    ...
    return {
      ...
      utilizationPercent: event?.capacity
        ? ((eventRegs.length / event.capacity) * 100).toFixed(1)
        : null,
    };
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">
                {((confirmedRegistrations.length / totalRegistrations) * 100).toFixed(1)}%
              </p>
            </div>
            ...
            <div key={cat.category} className="border rounded-lg p-4">
              ...
              <div className="mt-2 space-y-1">
                <p className="text-sm text-gray-600">
                  Events: <span className="font-medium">{cat.events}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Registrations: <span className="font-medium">{cat.registrations}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Revenue: <span className="font-medium text-green-600">₹{cat.revenue}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Avg/Event:{" "}
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="text-right">
                        ...
                        {stats.capacity && (
                          <p className="text-xs text-gray-500">
                            {stats.utilizationPercent}% filled
                          </p>
                        )}
                      </div>
                      <div className="text-right">
                  ...
                  <div className="p-6 border-t bg-white">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <div>
                            ...
                            <p className="text-sm">
                              Male: {stats.genderDist.Male}, Female: {stats.genderDist.Female}
                            </p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">Top Schools:</p>
                            {stats.topEventSchools.map(([school, count]) => (
                              <p key={school} className="text-sm">
                                {school}: {count}
                              </p>
                            ))}
                          </div>
                          ...
                          <div>
                            ...
                            <p className="text-sm">
                              1st: {stats.yearDist["1st"]}, 2nd: {stats.yearDist["2nd"]}, 3rd:{" "}
                              {stats.yearDist["3rd"]}, 4th: {stats.yearDist["4th"]}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <p className="text-sm">
                            <span className="text-gray-600">Fee:</span> ₹{event.fee}
                          </p>
                          <p className="text-sm">
                            <span className="text-gray-600">Venue:</span> {event.venue}
                          </p>
                          <p className="text-sm">
                          ...
                          {stats.capacity && (
                            <p className="text-sm">
                              <span className="text-gray-600">Capacity:</span> {stats.total}/
                              {stats.capacity}
                            </p>
                          )}
                          {stats.teamStats && (
                            <div className="mt-2">
                              ...
                              <p className="text-sm">
                                Range: {stats.teamStats.minSize}-{stats.teamStats.maxSize}
                              </p>
                            </div>
                          )}
                        </div>
